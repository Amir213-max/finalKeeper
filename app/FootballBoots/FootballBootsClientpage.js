    ...
    const result = products.filter((product) => {
      ...
      const attributesMatch = Object.entries(selectedAttributes).every(
        ([attrLabel, selectedVals]) => {
          ...
          const selectedLower = selectedVals.map((v) => v.toLowerCase());
          return attrs.some(
            (pav) =>
              pav.attribute?.label?.toLowerCase() === attrLabel.toLowerCase() &&
              selectedLower.includes(pav.key?.toLowerCase())
          );
        }
      );
    ...
    <div className="bg-[#373e3e] min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-0">
        ...
        <div className="lg:col-span-4 bg-white p-4 sm:p-6 rounded-t-2xl lg:rounded-none">
          ...
          {/* Brand Slider (hidden if only one brand) */}
          {brands.length > 1 && (
            <BrandsSlider
              brands={brands}
